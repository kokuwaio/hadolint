...
@build:
	docker build . --tag=kokuwaio/hadolint:dev --load

# Inspect image with docker.
@inspect: build
	docker image inspect kokuwaio/hadolint:dev

# Inspect image layers with `dive`.
@dive: build
	docker run --rm -it --volume=/var/run/docker.sock:/var/run/docker.sock:ro wagoodman/dive:latest kokuwaio/hadolint:dev
